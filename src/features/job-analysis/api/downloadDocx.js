...
// ─── Image crop helper ────────────────────────────────────────────────────────
//
// Center-crops imgData (Uint8Array) and renders at PHOTO_SCALE× the display
// size (~288 DPI) so the embedded image stays sharp when zoomed in Word.
// The ImageRun transformation still uses the 1× display dimensions, so layout
// is unchanged — only the embedded pixel density increases.
...
function cropToPhotoRatio(imgData) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = reject;
    img.onload = () => {
      const tgtW = PHOTO.wPX * PHOTO_SCALE;
      const tgtH = PHOTO.hPX * PHOTO_SCALE;
...
async function generateAnschreiben(text) {
  ...
  const children = [
    // Sender address — plain body text (DIN 5008: same size/colour as the rest of the letter)
    ...sender.map(s => new (d().Paragraph)({
      children: [rn(s, { color: COLOR.body })],
      spacing:  { before: 0, after: 20 },
    })),
    blk(),
